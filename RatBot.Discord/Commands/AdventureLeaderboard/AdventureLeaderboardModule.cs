    ...
    : InteractionModuleBase<IInteractionContext>
{
    ...
    public async Task ShowLeaderboardAsync(
        [Summary("channel", "Channel where RatBot should post the leaderboard.")]
        ITextChannel channel,
        [Summary("year", "Adventure event year to show.")]
        int year)
    {
    ...
    public async Task ExcludeAsync(
        [Summary("user", "User to exclude from the adventure leaderboard.")]
        IUser user)
    {
        await DeferAsync(true).ConfigureAwait(false);
    ...
    public async Task CreateThreadsAsync([Summary("channel_id", "Parent text channel ID.")] string channelId)
    {
        ...
        ITextChannel? channel = await Context.Guild
            .GetTextChannelAsync(parsedChannelId)
            .ConfigureAwait(false);
