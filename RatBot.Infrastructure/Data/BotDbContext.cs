...
public sealed class BotDbContext(DbContextOptions<BotDbContext> options)
    : DbContext(options),
        IUnitOfWork,
        IRepository<MetaSuggestionSettings>,
        IEmojiRepository,
        IMetaProposalRepository
{
    ...
    public IRepository<TAggregate> GetRepository<TAggregate>() =>
        ...
        ?? throw new NotSupportedException($"Repository for aggregate type {typeof(TAggregate).Name} is not supported by {nameof(BotDbContext)}.");
    ...
    public Task<MetaProposalState?> FindBySuggestionThreadAsync(ulong suggestionThreadChannelId, CancellationToken ct = default) =>
    ...
    public Task<MetaProposalState?> FindByProposalThreadAsync(ulong threadChannelId, CancellationToken ct = default) =>
        MetaProposalStates.FirstOrDefaultAsync(
            x => x.SuggestionThreadChannelId == threadChannelId || x.ProposalThreadChannelId == threadChannelId,
            ct
        );
    ...
    public async Task<IReadOnlyList<MetaProposalState>> FindExpiredPollsAsync(DateTimeOffset nowUtc, int limit, CancellationToken ct = default) =>
        await MetaProposalStates
            .Where(x =>
                x.Status == MetaProposalStatus.PollActive && x.PollExpiresAtUtc != null && x.PollExpiresAtUtc <= nowUtc && x.PollMessageId != null
            )
